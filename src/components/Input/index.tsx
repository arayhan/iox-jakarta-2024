	...
	return (
		<div className={clsxm("space-y-2", wrapperClassName)}>
			...
			<div className="relative">
				{prepend && (
					<div className="absolute left-3 top-1/2 -translate-y-1/2 transform text-[20px]">
						{prepend}
					</div>
				)}
				<input
				...
				/>
				{append && (
					<div className="absolute right-3 top-0 translate-y-1.5 transform text-[20px]">
						{append}
					</div>
				)}
			</div>
			{error?.text && (
				<div className={clsxm(error.className)}>{error.text}</div>
			)}
		</div>
	);
};
